...
def submit_github():
    ...
    if response.status_code == 200:
        ...
        for repo in repos:
            commits_response = requests.get(f"https://api.github.com/repos/{repo['full_name']}/commits")
            if commits_response.status_code == 200:
                ...
                commit_info = [{
                    ...
                    "date": datetime.strptime(commit["commit"]["author"]["date"], "%Y-%m-%dT%H:%M:%SZ").strftime("%B %d, %Y")
        ...
        return render_template("extension_hello.html", name=input_name, repos=repo_info)